...
class LazyFetchingArchitectureTest {
    ...
    @Autowired
    lateinit var artistGenreRepository: ArtistGenreRepository
    @Autowired
    ...
    fun `findById executes exactly one select`(): List<DynamicTest> {
        val cases = listOf(
            ...
            "ArtistGenreEntity" to { artistGenreRepository.findById(
                ArtistGenreId(
                    artist = "21bOoXa6JISSaqYu2oYbWy",
                    genre = "Folk"
                )
            ) },
            "UserArtistLinkEntity" to { userArtistLinkRepository.findById(
                UserArtistLinkId(
                    user = "3126nx54y24ryqyza3qxcchi4wry",
                    artist = "21bOoXa6JISSaqYu2oYbWy"
                )
            ) },
